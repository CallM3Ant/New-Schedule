...
// Single shared row holding everyone's templates.
// (One password, one user in mind — if you ever want per-user syncing,
// key rows by a user id instead of this fixed constant.)
const ROW_ID = "templates";
...
async function pushTemplates(templates) {
  const supabase = ensureClient();
  const { error } = await supabase
    .from("schedule_sync")
    .upsert({ id: ROW_ID, templates, updated_at: new Date().toISOString() });
...
window.ScheduleSync = { pushTemplates, pullTemplates };